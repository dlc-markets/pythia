...
struct OracleScheduler {
    oracle: Arc<Oracle>,
    scheduler: OracleSchedulerConfig,
    announcement_offset: Duration,
    event_transmitter: Sender<EventNotification>,
    error_transmitter: UnboundedSender<OracleSchedulerError>,
}
...
impl OracleScheduler {
    async fn create_scheduler_event(
        ...
        let announcement_offset = self.announcement_offset;
...
pub(crate) async fn create_events(
    oracle: Arc<Oracle>,
    config: OracleSchedulerConfig,
    event_transmitter: Sender<EventNotification>,
    error_transmitter: UnboundedSender<OracleSchedulerError>,
) -> Result<OracleScheduler> {
